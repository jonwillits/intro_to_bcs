...
class InterfaceFrame:
    ...
    def show_instance(self):
        ...
        try:
            self.parent.current_instance_index = int(self.shape_instance_entry.get())
        except ValueError:
            print("Invalid input. Please enter a valid integer.")
            return

        # Check if the index is in the valid range
        if 0 <= self.parent.current_instance_index < self.app.dataset.dataset_size:
            try:
                # Draw the matrix corresponding to the current index
                self.app.drawing_canvas.draw_matrix(self.app.dataset.x_list[self.app.current_instance_index])
            except IndexError:
