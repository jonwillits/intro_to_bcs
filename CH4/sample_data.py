...
class Dataset:
    ...
    def __init__(self, x1_range=10, x2_range=10):
    ...
    def random_dataset(self, n):
        ...
        # create self.x of size (2*n, 2), where the
        
    def plot_dataset(self, dataset):
        # Color map: blue (#0000ff) for y=1, orange (#ff7f0e) for y=0
        colors = {1: '#0000ff', 0: '#ff7f0e'}

        # Create a scatter plot
        plt.scatter(dataset['x1'], dataset['x2'], c=dataset['y'].map(colors), label="Dataset")
        ...
        blue_patch = mpatches.Patch(color='#0000ff', label='y = 1')
        orange_patch = mpatches.Patch(color='#ff7f0e', label='y = 0')
    ...
    def generate_random_dataset(self, n, ):
        # Generate n values for y=1 and n values for y=0
        y = np.array([1] * n + [0] * n)

        # Generate x1 and x2 values from a normal distribution with mean=0 and stdev=3, and convert to ints
        x1 = np.random.normal(random_mean, random_stdev, 2 * n).astype(int)
        x2 = np.random.normal(random_mean, random_stdev, 2 * n).astype(int)
...
def main():
    ...
    # Generate the dataset based on the category_type
    dataset = generate_dataset(n, category_type, category_mean, category_stdev, random_mean, random_stdev,
                               radius, noise)
